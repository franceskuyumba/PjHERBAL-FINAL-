...
export default function CartPage() {
  ...
  const applyCoupon = async () => {
    ...
    if (res.ok && data?.coupon) {
      ...
      toast(data?.error || "This coupon is not valid.", "error");
    }
  };
  ...
  if (items.length === 0) {
    return (
      <div className="container-site py-16 sm:py-24">
        <h1 className="mb-8 text-center font-display text-3xl font-bold text-brand-950">Your Cart</h1>
        <EmptyState
          icon={<ShoppingBag className="h-8 w-8" />}
          title="Your cart is empty"
          description="Looks like you haven't added anything yet. Explore our premium supplements and get started."
          action={
  ...
  return (
    <div className="container-site py-8 sm:py-12">
      <h1 className="font-display text-3xl font-bold text-brand-950">Your Cart</h1>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          {totals.subtotal < SHIPPING.freeThreshold && totals.subtotal > 0 && (
            <div className="rounded-2xl border border-gold-200 bg-gold-50 p-4">
              <p className="text-sm font-medium text-gold-800">
                Add {formatTZS(remaining)} more to unlock <strong>free delivery</strong> 🚚
              </p>
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-ink/45">{formatTZS(item.price)} each</p>
                    </div>
                    <button
                      onClick={() => {
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="card p-6">
            <h2 className="font-display text-xl font-bold text-brand-950">Order summary</h2>
            ...
            {coupon && (
              ...
              >
                {coupon.code} ({coupon.type === "PERCENTAGE" ? `${coupon.value}%` : formatTZS(coupon.value)} off)
            ...
            <dl className="mt-5 space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-semibold text-ink">
                  {totals.shipping === 0 ? <span className="text-brand-600">FREE</span> : formatTZS(totals.shipping)}
                </dd>
              </div>
