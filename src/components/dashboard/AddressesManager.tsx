...
export function AddressesManager({ addresses }: { addresses: Address[] }) {
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(data.error || t("dash.addresses.saveError"));
        return;
      }
      setForm(empty);
      setEditingId(null);
      ...
      setErrorMsg(t("dash.addresses.networkError"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async (id: string) => {
    if (!confirm(t("dash.addresses.deleteConfirm"))) return;
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-brand-950">{t("dash.addresses.title")}</h1>
      <p className="mt-1 text-sm text-ink/55">{t("dash.addresses.subtitle")}</p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        {items.length === 0 && (
          <div className="rounded-3xl border border-dashed border-ink/15 p-8 text-center sm:col-span-2">
            ...
            <p className="mt-3 text-sm font-semibold text-brand-950">{t("dash.addresses.emptyTitle")}</p>
            <p className="mt-1 text-xs text-ink/50">{t("dash.addresses.emptyDesc")}</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 rounded-3xl border border-ink/5 bg-white p-6 shadow-card">
        <h2 className="font-display text-lg font-bold text-brand-950">
          {editingId ? t("dash.addresses.editTitle") : t("dash.addresses.addTitle")}
        </h2>
        {errorMsg && (
        ...
        <form onSubmit={onSave} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label={t("dash.addresses.labelField")}>
            <Input placeholder={t("dash.addresses.labelPlaceholder")} value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} />
          </Field>
          <Field label={t("dash.addresses.recipientName")}>
            <Input
              required
              placeholder={t("dash.addresses.recipientPlaceholder")}
              value={form.recipientName}
              onChange={(e) => setForm({ ...form, recipientName: e.target.value })}
            />
          </Field>
          <Field label={t("dash.addresses.phone")}>
            <Input
              required
              type="tel"
              placeholder={t("dash.addresses.phonePlaceholder")}
              value={form.phone}
              onChange={(e) => setForm({ ...form, phone: e.target.value })}
            />
          </Field>
          <Field label={t("dash.addresses.region")}>
            <select
              required
              value={form.region}
            ...
            >
              <option value="">{t("dash.addresses.selectRegion")}</option>
          ...
          <Field label={t("dash.addresses.district")}>
            <Input required placeholder={t("dash.addresses.districtPlaceholder")} value={form.district} onChange={(e) => setForm({ ...form, district: e.target.value })} />
          </Field>
          <Field label={t("dash.addresses.street")}>
            <Input required placeholder={t("dash.addresses.streetPlaceholder")} value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} />
          </Field>
          <div className="flex gap-3 sm:col-span-2">
            <Button type="submit" loading={loading}>
              {editingId ? t("dash.addresses.saveChanges") : t("dash.addresses.addBtn")}
            </Button>
            {editingId && (
              <Button
                type="button"
